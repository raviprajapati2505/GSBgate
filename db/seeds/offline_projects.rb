  ...
  certification_path = 
    project.
      ...
      find_or_initialize_by(
        name: row["Certification Name"]&.strip,
    ...
    certification_path.certified_at = row["Certification Certified At"]
  ...
  if (['v2.0', 'v2.1'].include?(name))
    ...
    (2..@xlsx.last_row).each do |i|
      ...
      begin
        ...
        # create scheme_mix_criteria
        create_scheme_mix_criteria(i, @xlsx.row(i), scheme_mix) if scheme_mix.persisted?

      rescue => e
        @errors << "Sheet: #{name}, Row: #{i}, General Error: #{e.message}"
      end
    end
