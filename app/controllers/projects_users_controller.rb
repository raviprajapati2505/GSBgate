      ...
      ProjectsUser.transaction do
        params[:projects_users].each do |pu|
          ...
          if pu.key?("user_id") && pu.key?("role") && pu.key?("certification_team_type") 
            ...
            if can?(:create, projects_user) && (!projects_user.gsb_team? || projects_user.user.gord_employee?)
              projects_user.save!
  ...
  def list_users_sharing_projects
    if params.has_key?(:user_id) && params.has_key?(:q) && params.has_key?(:page)
      if params[:user_id] == current_user.id.to_s
        if current_user.is_system_admin? || current_user.is_gsb_top_manager? || current_user.is_gsb_manager? || current_user.is_gsb_admin?
