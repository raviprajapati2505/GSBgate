class Api::V1::ProjectsController < Api::ApiController
  ...
  def index
    ...
    query = Project.accessible_by(current_ability).joins(certification_paths: [:certificate])
                .where(certification_paths: {certification_path_status_id: [CertificationPathStatus::CERTIFIED, CertificationPathStatus::CERTIFICATE_IN_PROCESS]}, certificates: {certificate_type: Certificate::certificate_types[:operations_type], gsb_version: '2019'})
                .where("NOT EXISTS(#{SchemeMixCriterionEpl.joins(scheme_mix_criterion: [:scheme_mix]).where('scheme_mixes.certification_path_id = certification_paths.id').where('scheme_mix_criterion_performance_labels.band IS NULL OR scheme_mix_criterion_performance_labels.band = \'-\'').unscope(:order).to_sql})")
                .where("NOT EXISTS(#{SchemeMixCriterionWpl.joins(scheme_mix_criterion: [:scheme_mix]).where('scheme_mixes.certification_path_id = certification_paths.id').where('scheme_mix_criterion_performance_labels.band IS NULL OR scheme_mix_criterion_performance_labels.band = \'-\'').unscope(:order).to_sql})")
    total_as_built_energy_consumption_query = SchemeMixCriterionEpl.accessible_by(current_ability).joins(scheme_mix_criterion: [scheme_mix: [:scheme, certification_path: [:certificate, :project]]])
                                                  .where('scheme_criterion_performance_labels.label = \'As Built\'')
                                                  .where(certification_paths: {certification_path_status_id: [CertificationPathStatus::CERTIFIED, CertificationPathStatus::CERTIFICATE_IN_PROCESS]}, certificates: {certificate_type: Certificate::certificate_types[:operations_type], gsb_version: '2019'})
                                                  .where("NOT EXISTS(SELECT * FROM scheme_mix_criterion_performance_labels smcpl LEFT JOIN scheme_mix_criteria smc ON smc.id = smcpl.scheme_mix_criterion_id LEFT JOIN scheme_mixes sm ON sm.id = smc.scheme_mix_id WHERE (smcpl.band IS NULL OR smcpl.band = '-') AND sm.certification_path_id = certification_paths.id)")
                                                  .unscope(:order)
    total_as_operated_energy_consumption_query = SchemeMixCriterionEpl.accessible_by(current_ability).joins(scheme_mix_criterion: [scheme_mix: [:scheme, certification_path: [:certificate, :project]]])
                                                     .where('scheme_criterion_performance_labels.label = \'As Operated\'')
                                                     .where(certification_paths: {certification_path_status_id: [CertificationPathStatus::CERTIFIED, CertificationPathStatus::CERTIFICATE_IN_PROCESS]}, certificates: {certificate_type: Certificate::certificate_types[:operations_type], gsb_version: '2019'})
                                                     .where("NOT EXISTS(SELECT * FROM scheme_mix_criterion_performance_labels smcpl LEFT JOIN scheme_mix_criteria smc ON smc.id = smcpl.scheme_mix_criterion_id LEFT JOIN scheme_mixes sm ON sm.id = smc.scheme_mix_id WHERE (smcpl.band IS NULL OR smcpl.band = '-') AND sm.certification_path_id = certification_paths.id)")
                                                     .unscope(:order)

    total_as_built_water_consumption_query = SchemeMixCriterionWpl.accessible_by(current_ability).joins(scheme_mix_criterion: [scheme_mix: [:scheme, certification_path: [:certificate, :project]]])
                                                 .where('scheme_criterion_performance_labels.label = \'As Built\'')
                                                 .where(certification_paths: {certification_path_status_id: [CertificationPathStatus::CERTIFIED, CertificationPathStatus::CERTIFICATE_IN_PROCESS]}, certificates: {certificate_type: Certificate::certificate_types[:operations_type], gsb_version: '2019'})
                                                 .where("NOT EXISTS(SELECT * FROM scheme_mix_criterion_performance_labels smcpl LEFT JOIN scheme_mix_criteria smc ON smc.id = smcpl.scheme_mix_criterion_id LEFT JOIN scheme_mixes sm ON sm.id = smc.scheme_mix_id WHERE (smcpl.band IS NULL OR smcpl.band = '-') AND sm.certification_path_id = certification_paths.id)")
                                                 .unscope(:order)
    total_as_operated_water_consumption_query = SchemeMixCriterionWpl.accessible_by(current_ability).joins(scheme_mix_criterion: [scheme_mix: [:scheme, certification_path: [:certificate, :project]]])
                                                    .where('scheme_criterion_performance_labels.label = \'As Operated\'')
                                                    .where(certification_paths: {certification_path_status_id: [CertificationPathStatus::CERTIFIED, CertificationPathStatus::CERTIFICATE_IN_PROCESS]}, certificates: {certificate_type: Certificate::certificate_types[:operations_type], gsb_version: '2019'})
                                                    .where("NOT EXISTS(SELECT * FROM scheme_mix_criterion_performance_labels smcpl LEFT JOIN scheme_mix_criteria smc ON smc.id = smcpl.scheme_mix_criterion_id LEFT JOIN scheme_mixes sm ON sm.id = smc.scheme_mix_id WHERE (smcpl.band IS NULL OR smcpl.band = '-') AND sm.certification_path_id = certification_paths.id)")
                                                    .unscope(:order)
  ...
  def typologies
    query = Scheme.joins(development_types: [:certificate]).where(certificates: {certificate_type: Certificate::certificate_types[:operations_type]}, gsb_version: '2019')
