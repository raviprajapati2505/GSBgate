...
const chart = (projData, propName) => {
  ...
  const formatting = (name, d) => {
    if (Array.isArray(d)) {
      return ({
        name,
        value: propName === "" ? d.length : d.reduce((a, b) => a + b[propName], 0)
      }) //children: d.map(p=>{return {name:p["Project ID"], value:p.cArea, children:[]}})})
    } else {
      let sorted = Object.keys(d).sort();
      return ({
        name,
        children: sorted.map(k => formatting(k, d[k]))
      })
    }
  }
  ...
  var path = g.append("g")
    ...
    .attr("fill", d => {
      ...
      return color(d.data.name, d.parent.data);
    })
