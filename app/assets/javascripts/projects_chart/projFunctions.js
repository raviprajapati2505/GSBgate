...
const certfictionStage = (project) => {
  let certifiedDescriptions = ["Certified"]

  switch (project["Certification Type"]) {
    case "GSB-CM":
      if (project["Certification Stage"] === "GSB Construction Management Certificate") {
        return 4;
      } else {
...
const trimNonValid = (arr, year) => {
  ...
  let res = arr.filter((p) => {
    ...
    //same for OP
    //if (awarded>=year && p["Certification Type"]==="GSB-OP" && stage===3 ) return false
  ...
  res.forEach(p => {
    ...
    if (awarded >= year && stage < 4 && p["Certification Type"] === "GSB-CM") {
      p["Certification Submission Status"] = "Submitting"
      p["Certification Year"] = ""
      p["Certification Awarded On"] = new Date("1-Jan-2010")
      p["Certification Rating"] = ""
    ...
    if (stage < 4 && p["Certification Type"] === "GSB-CM") {
      p["Certification Submission Status"] = "Submitting"
      p["Certification Year"] = ""
      p["Certification Awarded On"] = new Date("1-Jan-2010")
      p["Certification Rating"] = ""
    ...
    //OP
    if (awarded >= year && p["Certification Type"] === "GSB-OP" && stage === 3) {
      p["Certification Submission Status"] = "Submitting"
      p["Certification Year"] = ""
      p["Certification Awarded On"] = new Date("1-Jan-2010")
      p["Certification Rating"] = ""
...
const prepareAreaAttr = (projArr) => {
  return projArr.map(p => {
    ...
    let pArea = (scheme === "Dis" || scheme === "Par" || scheme === "Nei" || type === "GSB-CM") ? parseNumber(p["Project Plot Area"]) : 0;

    let cArea = 0;
    if (scheme === "Nei") {
      cArea = parseNumber(p["Project Gross Built Up Area"]);
    } else if (scheme !== "Dis" && scheme !== "Par" && type !== "GSB-CM") {
      cArea = parseNumber(p["Project Certified Area"]);
    }

    let pNumber = (scheme === "Dis" || scheme === "Par" || scheme === "Nei" || type === "GSB-CM")
    let cNumber = !pNumber
