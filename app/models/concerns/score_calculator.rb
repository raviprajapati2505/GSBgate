  ...
  module ClassMethods
    ...
    def build_check_scheme_mix_criterion_state_query(field_name)
      ...
      if field_name == :achieved_score || SchemeMixCriterion::ACHIEVED_SCORE_ATTRIBUTES.include?(field_name.to_s)
        ...
        # if 'verifying' or 'verifying after appeal', return true for gsb admins and managers, or project certifiers, as they are allowed to see the achieved scores
        ...
        user_is_manager = 'EXISTS(SELECT id FROM users WHERE id = %{user_id} AND role IN (%{user_roles}))' % {user_id: User.current.id, user_roles: [User.roles[:system_admin], User.roles[:gsb_top_manager], User.roles[:gsb_manager], User.roles[:gsb_admin]].join(', ')}
