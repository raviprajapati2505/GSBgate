  ...
  module Datatables
    class OfflineProjects < Effective::Datatable
      ...
      datatable do
        ...
        col :certification_name, col_class: 'multiple-select', sql_column: 'offline_certification_paths.name', label: t('models.effective.datatables.offline.certification_path.name'), search: { as: :select, collection: Proc.new { get_certificate_types_names(current_user) } } do |rec| link_to_if(!current_user.record_checker?,
        ...
        col :certification_version, col_class: 'multiple-select', sql_column: 'offline_certification_paths.version', label: t('models.effective.datatables.offline.certification_path.version'), search: { as: :select, collection: Proc.new { Offline::CertificationPath.versions.map { |k, v| [k, k] } } } do |rec|
          link_to_if(!current_user.record_checker?,
        ...
        col :certification_rating, partial: '/offline/certification_paths/rating', sql_column: 'offline_certification_paths.rating', partial_as: 'rec', col_class: 'multiple-select', label: t('models.effective.datatables.offline.certification_path.rating'), search: { as: :select, collection: Proc.new { Offline::CertificationPath.ratings.map { |k, v| [k.titleize, k] }.uniq } } 
