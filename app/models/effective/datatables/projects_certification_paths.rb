  ...
  module Datatables
    class ProjectsCertificationPaths < Effective::Datatable
      ...
      datatable do
        ...
        table_column 'certificate_id', column: 'certificates.id', label: 'Certificate', filter: {type: :select, values: Proc.new { Certificate.all.order(:display_weight).map { |certificate| [certificate.name + ' ' + certificate.gsas_version, certificate.id] } }} do |rec|
        ...
        table_column 'schemes_array', label: 'Typologies', column: "ARRAY_TO_STRING(ARRAY(SELECT CONCAT(schemes.name, ' ', schemes.gsas_version) from schemes INNER JOIN scheme_mixes ON schemes.id = scheme_mixes.scheme_id WHERE scheme_mixes.certification_path_id = certification_paths.id), '|||')" do |rec|
      ...
      def collection
        coll = Project
                   ...
                   .select("ARRAY_TO_STRING(ARRAY(SELECT CONCAT(schemes.name, ' ', schemes.gsas_version) from schemes INNER JOIN scheme_mixes ON schemes.id = scheme_mixes.scheme_id WHERE scheme_mixes.certification_path_id = certification_paths.id), '|||') AS schemes_array")
