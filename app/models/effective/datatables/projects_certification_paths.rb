  ...
  module Datatables
    class ProjectsCertificationPaths < Effective::Datatable
      ...
      datatable do
        ...
        table_column 'owner_email', column: 'owner.email'

        #table_column 'certification_path_id', column: 'certification_paths.id', type: :integer, label: 'Certificate ID'
        table_column 'certificate_id', column: 'certificates.id', label: 'Certificate', filter: {type: :select, values: Proc.new { Certificate.all.order(:display_weight).map { |certificate| [certificate.name + ' ' + certificate.version, certificate.id] } }} do |rec|
        ...
        table_column 'certification_path_pcr_track', column: 'certification_paths.pcr_track', type: :boolean, visible: false
        table_column 'certification_path_pcr_track_allowed', column: 'certification_paths.pcr_track_allowed', type: :boolean, visible: false
        table_column 'certification_path_development_type', column: 'certification_paths.development_type', type: :integer, visible: true, filter: {type: :select, values: Proc.new { CertificationPath.development_types.map { |k| [k[0].humanize, k[1]] } }} do |rec|
        ...
        table_column 'certification_path_appealed', column: 'certification_paths.appealed', type: :boolean, visible: false
        table_column 'certification_path_created_at', column: 'certification_paths.created_at', type: :datetime, visible: false, filter: {type: :select, values: Proc.new { CertificationPath.pluck_date_field_by_year_month_day(:created_at, :desc) }}
        table_column 'certification_path_started_at', column: 'certification_paths.started_at', type: :datetime, visible: false, filter: {type: :select, values: Proc.new { CertificationPath.pluck_date_field_by_year_month_day(:started_at, :desc) }}
        table_column 'certification_path_certified_at', column: 'certification_paths.certified_at', type: :datetime, visible: false, filter: {type: :select, values: Proc.new { CertificationPath.pluck_date_field_by_year_month_day(:certified_at, :desc) }}
        table_column 'certification_path_duration', column: 'certification_paths.duration', type: :integer, visible: false
        ...
        table_column 'certification_path_status_is_active', column: 'CASE WHEN certification_path_statuses.id IS NULL THEN false WHEN certification_path_statuses.id = 15 THEN false WHEN certification_path_statuses.id = 16 THEN false ELSE true END', type: :boolean, label: 'Active'

        # table_column 'projects_users_emails', column: "string_agg(users.email, '|||')", type: :string do |rec|
        #   rec.projects_users_emails.split('|||').sort.uniq.join('<br/>').html_safe unless rec.projects_users_emails.nil?
        # end
        # table_column 'projects_users_count', column: 'count(projects_users.user_id)'

        # array_column :score, sortable: false, filter: false do |rec|
        #   if rec.certification_path_id.present?
        ...
        table_column 'schemes_array', label: 'Typologies', column: "ARRAY_TO_STRING(ARRAY(SELECT CONCAT(schemes.name, ' ', schemes.version) from schemes INNER JOIN scheme_mixes ON schemes.id = scheme_mixes.scheme_id WHERE scheme_mixes.certification_path_id = certification_paths.id), '|||')" do |rec|
          rec.schemes_array.split('|||').sort.join('<br/>').html_safe unless rec.schemes_array.nil?
        end
        table_column 'assessors_array', label: 'Project Team', column: "ARRAY_TO_STRING(ARRAY(SELECT assessor_users.email FROM users as assessor_users INNER JOIN projects_users as assessor_project_users ON assessor_project_users.user_id = assessor_users.id  WHERE assessor_project_users.role IN (0,1) AND assessor_project_users.project_id = projects.id), '|||')" do |rec|
          rec.assessors_array.split('|||').sort.join('<br/>').html_safe unless rec.assessors_array.nil?
        end
        table_column 'certifiers_array', label: 'Certifier Team', column: "ARRAY_TO_STRING(ARRAY(SELECT certifier_users.email FROM users as certifier_users INNER JOIN projects_users as certifier_project_users ON certifier_project_users.user_id = certifier_users.id  WHERE certifier_project_users.role IN (3,4) AND certifier_project_users.project_id = projects.id), '|||')" do |rec|
          rec.certifiers_array.split('|||').sort.join('<br/>').html_safe unless rec.certifiers_array.nil?
        end
        table_column 'enterprise_clients_array', label: 'Enterprise Clients', column: "ARRAY_TO_STRING(ARRAY(SELECT enterprise_client_users.email FROM users as enterprise_client_users INNER JOIN projects_users as enterprise_client_project_users ON enterprise_client_project_users.user_id = enterprise_client_users.id  WHERE enterprise_client_project_users.role IN (2) AND enterprise_client_project_users.project_id = projects.id), '|||')" do |rec|
      ...
      def collection
        coll = Project
                   ...
                   .select("CONCAT(certificates.name, ' ', certificates.version) as certificate_name")
                   ...
                   .select("ARRAY_TO_STRING(ARRAY(SELECT CONCAT(schemes.name, ' ', schemes.version) from schemes INNER JOIN scheme_mixes ON schemes.id = scheme_mixes.scheme_id WHERE scheme_mixes.certification_path_id = certification_paths.id), '|||') AS schemes_array")
        ...
        # .select("string_agg(users.email, '|||') as projects_users_emails")
        # .select('count(projects_users.user_id) as projects_users_count')

        # use cancan ability to limit the visisble projects
        coll = coll.accessible_by(current_ability) if current_ability.present?
        return coll
      end
