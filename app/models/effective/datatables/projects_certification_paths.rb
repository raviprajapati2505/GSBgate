    ...
    class ProjectsCertificationPaths < Effective::Datatable
      ...
      def collection
        coll = Project
                   ...
                   .select("ARRAY_TO_STRING(ARRAY(SELECT project_team_users.name FROM users as project_team_users INNER JOIN projects_users as project_team_project_users ON project_team_project_users.user_id = project_team_users.id  WHERE project_team_project_users.role IN (5,1) AND project_team_project_users.project_id = projects.id), '|||') AS project_team_array")
                   .select("ARRAY_TO_STRING(ARRAY(SELECT cgp_project_managers.name FROM users as cgp_project_managers INNER JOIN projects_users as cgp_project_managers_project_users ON cgp_project_managers_project_users.user_id = cgp_project_managers.id  WHERE cgp_project_managers_project_users.role IN (1) AND cgp_project_managers_project_users.project_id = projects.id), '|||') AS cgp_project_manager_array")
                   .select("ARRAY_TO_STRING(ARRAY(SELECT gsas_trust_team_users.name FROM users as gsas_trust_team_users INNER JOIN projects_users as gsas_trust_team_project_users ON gsas_trust_team_project_users.user_id = gsas_trust_team_users.id  WHERE gsas_trust_team_project_users.role IN (3,4) AND gsas_trust_team_project_users.project_id = projects.id), '|||') AS gsas_trust_team_array")
