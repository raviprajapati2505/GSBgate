  ...
  module Datatables
    class ProjectsCertificationPaths < Effective::Datatable
      ...
      datatable do
        ...
        table_column 'schemes_array', label: 'Certificate Typologies', column: "ARRAY_TO_STRING(ARRAY(SELECT CONCAT(schemes.name, ' ', schemes.gsas_version) from schemes INNER JOIN scheme_mixes ON schemes.id = scheme_mixes.scheme_id WHERE scheme_mixes.certification_path_id = certification_paths.id), '|||')" do |rec|
